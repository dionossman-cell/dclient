...
public class BlockESPScreen extends Screen {
    ...
    public boolean keyPressed(KeyEvent event) {
        ...
        // Convert printable keys to characters
    ...
    public void render(GuiGraphics gfx, int mouseX, int mouseY, float delta) {
        ...
        gfx.drawCenteredString(font, "Block ESP - Select Blocks", width / 2, PADDING, 0xFFFFFFFF);
        ...
        for (int row = scrollOffset; row < Math.min(scrollOffset + vr, (filtered.size() + COLS - 1) / COLS); row++) {
            for (int col = 0; col < COLS; col++) {
                ...
                boolean enabled = module.enabledBlocks.contains(name);
        ...
        gfx.drawString(font, "Enabled: " + module.enabledBlocks.size() + " / " + filtered.size(), PADDING, HEADER_H - 10, 0xFF888888, false);
    ...
    private void handleClick(int mx, int my) {
        ...
        for (int row = scrollOffset; row < Math.min(scrollOffset + vr, (filtered.size() + COLS - 1) / COLS); row++) {
            for (int col = 0; col < COLS; col++) {
                ...
                if (mx >= bx && mx < bx + COL_W && my >= by && my < by + ROW_H) {
                    if (module.enabledBlocks.contains(name)) module.enabledBlocks.remove(name);
                    else module.enabledBlocks.add(name);
                    return;
                }
            }
        }
        ...
        if (my >= btnY && my < btnY + 18) {
            if (mx >= bx0 && mx < bx0 + btnW) module.enabledBlocks.addAll(filtered);
            else if (mx >= bx0 + btnW + gap && mx < bx0 + btnW * 2 + gap) module.enabledBlocks.removeAll(filtered);
